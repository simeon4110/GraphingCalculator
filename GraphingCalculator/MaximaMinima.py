"""Program to find the minima/maxima of an coordinate array.

Step one of this program uses list segments to separate the x and y coordinates
in the list passed thru from the function call. Then, using a while loop, the
...
this solution took me many days, I lost track of all my sources. I wrote this
final form of the program with no outside reference.
"""
...
ELLIPSE_W_H = 6 # width and heigh of the ellipse markers
...
def maxima_minima(array):
    ...
    for item in minima_array: # draw the minima
        ...
        if round(x) != (X_ZER0 + X_OFFSET): # 401 to account for rounding errors
            x = x - (ELLIPSE_W_H / 2) ##
            y = y - (ELLIPSE_W_H / 2) # account for width/height of ellipse
            setColor("purple")
            ellipse(x, y, ELLIPSE_W_H, ELLIPSE_W_H)
